...
import torch
from utils.dataset import RGBDataset
from torch.utils.data import DataLoader
from fas_models.base.resnet_ import ResNet18_ as Net
...
def train(Model=Net, name='rgb-resnet18', verbose=1, batch_size=60, do_validation=True, protocol=1,
          data_parallel=False, ema=None, **model_kwargs):
    ...
    if not do_validation:
        valloader = None
    model = Model(in_ch=3, **model_kwargs)
    ...
    trainer = ModelWrapper(model, device, ema=ema)
    trainer.compile(
        optimizer=optim,
        loss_fcn=loss_fcn,
    ...
    rtv = trainer.evaluate(testloader, verbose=verbose, using_ema=True)
