...
def build_dataset(batch_size=16, image_size=128, crop_scale=(0.5, 0.8), protocol=1):
    trainset = HSIDataset(
        root_dir='./data/datasets/ssi_reconstructed',
...
def train(Model=Net, name='hsi-resnet18', verbose=1, batch_size=60, do_validation=False, protocol=1,
    ...
    if not do_validation:
        valloader = None
    model = Model(in_ch=30, **model_kwargs)
